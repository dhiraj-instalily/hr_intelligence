...
def main():
    ...
    if args.remove_duplicates:
        if args.dry_run:
            ...
            """)
            duplicate_count = db_handler.cursor.fetchone()[0]
            logger.info(f"Would remove {duplicate_count} duplicate documents")
        else:
            logger.info("Removing duplicate documents")
            # Get count before deletion
            db_handler.cursor.execute("SELECT COUNT(*) FROM documents")
            before_count = db_handler.cursor.fetchone()[0]
            ...
            db_handler.cursor.execute("SELECT COUNT(*) FROM documents")
            after_count = db_handler.cursor.fetchone()[0]
