...
The server will start and listen for MCP requests on the specified port.
"""
...
from src.database.schema import SearchQuery, SearchResult, Candidate
...
mcp = FastMCP("HR Intelligence")

# Create a FastAPI app
app = FastAPI(title="HR Intelligence API")
...
def search_by_role_impl(keywords: str, company: str = None) -> List[Dict[str, Any]]:
    """
...
def semantic_search_experience_impl(query: str) -> List[Dict[str, Any]]:
    """
...
def find_skill_combinations_impl(skills: List[str], match_all: bool = False) -> List[Dict[str, Any]]:
    """
...
def get_candidate_details_impl(candidate_id: str) -> Dict[str, Any]:
    """
...
def search_by_education_impl(institution: str = None, degree: str = None) -> List[Dict[str, Any]]:
    """
...
def search_by_role(keywords: str, company: str = None) -> List[Dict[str, Any]]:
    ...
    Returns:
        List of candidates matching the search criteria, with their details and match scores
    """
    return search_by_role_impl(keywords, company)
...
def semantic_search_experience(query: str) -> List[Dict[str, Any]]:
    ...
    Returns:
        List of candidates with semantically matching experiences, ranked by relevance
    """
    return semantic_search_experience_impl(query)
...
def find_skill_combinations(skills: List[str], match_all: bool = False) -> List[Dict[str, Any]]:
    ...
    Returns:
        List of candidates matching the skill criteria, with their details and match scores
    """
    return find_skill_combinations_impl(skills, match_all)
...
def get_candidate_details(candidate_id: str) -> Dict[str, Any]:
    ...
    Returns:
        Complete candidate profile with all available information
    """
    return get_candidate_details_impl(candidate_id)
...
def search_by_education(institution: str = None, degree: str = None) -> List[Dict[str, Any]]:
    ...
    Returns:
        List of candidates matching the education criteria
    """
    return search_by_education_impl(institution, degree)

# Define FastAPI endpoints for the tools
@app.post("/tools/search_by_role")
async def api_search_by_role(keywords: str, company: str = None):
    """API endpoint for search_by_role tool."""
    return search_by_role_impl(keywords, company)

@app.post("/tools/semantic_search_experience")
async def api_semantic_search_experience(query: str):
    """API endpoint for semantic_search_experience tool."""
    return semantic_search_experience_impl(query)

@app.post("/tools/find_skill_combinations")
async def api_find_skill_combinations(skills: List[str], match_all: bool = False):
    """API endpoint for find_skill_combinations tool."""
    return find_skill_combinations_impl(skills, match_all)

@app.post("/tools/get_candidate_details")
async def api_get_candidate_details(candidate_id: str):
    """API endpoint for get_candidate_details tool."""
    return get_candidate_details_impl(candidate_id)

@app.post("/tools/search_by_education")
async def api_search_by_education(institution: str = None, degree: str = None):
    """API endpoint for search_by_education tool."""
    return search_by_education_impl(institution, degree)

# Create a dummy data endpoint for testing
@app.get("/test")
async def test_endpoint():
    """Test endpoint to verify the server is running."""
    return {"status": "ok", "message": "HR Intelligence MCP Server is running"}
